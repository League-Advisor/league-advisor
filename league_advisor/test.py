    ...
    def prompt_user(self):
            ...
            while not len(self.team) == 5:
                if self.mode == "c":
                    ...
                    print()
                    print(f"{color.YELLOW}[Support,Adc,Mid,Jungle,Top]{color.RESET}")
                    print()
                ...
                if not len  (self.team) == 5:
                    if self.mode == "c":
                        print(f"{color.RED}Please enter a valid composition.{color.RESET}")
                    else:
                    ...
                    while True:
                        if self.mode == "c":
                            print()
                            print(f"{color.YELLOW}Enter your current champion name.{color.RESET}")
                            print()
                        else:
                            print()
                        ...
                        if not self.user_response.lower().strip() in [name.lower() for name in self.team] or not self.user_response.lower().strip() in strings["champion_list_lower"]:
                            if self.mode == "c":
                                print(f"{color.RED}Enter your champion name correctly.{color.RESET}")
                            else:
                    ...
                    while not len(self.enemy) == 5:
                        ...
                        for i in range(len(self.enemy)):
                            if self.enemy[i].lower().strip() in [name.lower() for name in self.team]:
                                if self.mode == "c":
                                    print(f"{color.RED}Please enter a valid composition. One champion cannot be on both teams.{color.RESET}")
                                else:
